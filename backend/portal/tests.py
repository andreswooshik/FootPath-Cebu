...
def _license_file():
    ...
    return SimpleUploadedFile('license.pdf', pdf_bytes(), content_type='application/pdf')
...
class CoordinatorSignupTests(TestCase):
    ...
    def test_license_rejects_oversize(self):
        ...
        from accounts.validators import COACH_LICENSE_MAX_BYTES
    ...
    def test_pending_coordinator_cannot_login_until_approved(self):
        ...
        self.assertFalse(self.client.login(username='pending@club.test', password=_PASSWORD))
        user = User.objects.get(email='pending@club.test')
        user.is_active = True
        user.save(update_fields=['is_active'])
        self.assertTrue(self.client.login(username='pending@club.test', password=_PASSWORD))
...
class SignupHardeningTests(TestCase):
    ...
    def test_license_rejects_forged_signature(self):
        ...
        resp = self.client.post(
            reverse('portal:signup'),
            _signup_data(
                email='forge@club.test',
                club_name='Forge FC',
                coach_license=forged,
            ),
        )
        self.assertEqual(resp.status_code, 200)  # re-rendered with an error
    ...
    def test_signup_is_rate_limited_per_ip(self):
        ...
        for _ in range(5):
            self.assertEqual(self.client.post(reverse('portal:signup'), {}).status_code, 200)
        self.assertEqual(self.client.post(reverse('portal:signup'), {}).status_code, 429)
...
class IndependentClubRosterTests(TestCase):
    ...
    def test_mobile_sidebar_has_no_close_button(self):
        coordinator, _club = make_coordinator(
            email='sidebar@club.test',
            club_name='Sidebar FC',
            is_school_affiliated=False,
        )
        self.client.force_login(coordinator)
...
class CreateAccountTests(TestCase):
    ...
    def test_create_player_without_email_creates_guardian_managed_profile(self):
        guardian = User.objects.create(
            username='managed-parent@club.test',
            email='managed-parent@club.test',
            role=Roles.GUARDIAN,
            club=self.club,
            firebase_uid='managed-parent-uid',
        )
        response = self.client.post(
            reverse('portal:create-account'),
            {
    ...
    def test_player_email_cannot_reuse_guardian_login_email(self):
        guardian = User.objects.create(
            ...
            role=Roles.GUARDIAN,
            club=self.club,
            firebase_uid='same-email-parent-uid',
        )
        response = self.client.post(
            reverse('portal:create-account'),
            {
    ...
    def test_coordinator_can_reset_player_privacy_pin(self):
        ...
        PlayerPrivacyPin.objects.create(
            player=player,
            pin_hash='argon2$placeholder',
            failed_attempts=3,
        )
        response = self.client.post(reverse('portal:player-pin-reset', args=[player.id]))
...
class GuardianLinkManagementTests(TestCase):
    ...
    def test_coordinator_links_and_unlinks(self):
        ...
        link = GuardianLink.objects.get(guardian=self.guardian, player=self.player)

        resp = self.client.post(reverse('portal:guardian-unlink', args=[link.pk]))
        self.assertRedirects(resp, reverse('portal:guardians'))
    ...
    def test_already_linked_players_are_hidden_from_picker(self):
        other_guardian = User.objects.create(
            username='other-g@club.test',
            email='other-g@club.test',
            role=Roles.GUARDIAN,
            club=self.club,
            firebase_uid='uid-other-g',
        )
    ...
    def test_cannot_unlink_another_clubs_link(self):
        ...
        link = GuardianLink.objects.create(guardian=other_guardian, player=other_player)
        resp = self.client.post(reverse('portal:guardian-unlink', args=[link.pk]))
