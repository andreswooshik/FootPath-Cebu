...
def complete_tournament_fixture(*, fixture, actor, payload):
    ...
    if fixture.completed_match_id or fixture.status == FixtureStatus.COMPLETED:
        raise ValidationError({'fixture': 'This fixture already has a recorded result.'})
    if fixture.status != FixtureStatus.SCHEDULED:
        raise ValidationError({'fixture': 'Only a scheduled fixture can record a result.'})
    if not fixture.schedule.is_published:
        raise ValidationError({'fixture': 'Publish the tournament before recording a result.'})
    if fixture.age_bracket_id is None:
        raise ValidationError({'fixture': 'The fixture needs an age bracket before recording.'})
    if fixture.opponent.strip().upper() == 'TBD':
        raise ValidationError({'fixture': 'Set the opponent before recording a result.'})
    kickoff = fixture.kickoff_at
    played_on = timezone.localtime(kickoff).date() if timezone.is_aware(kickoff) else kickoff.date()
    if played_on > timezone.localdate():
        raise ValidationError({'fixture': 'A result can be recorded only after match day.'})
    ...
    if squad is None:
        raise ValidationError(
            {'participants': ("The Coach must publish this age bracket's squad first.")}
        )
    participant_rows = payload['participants']
    player_ids = [row['playerId'] for row in participant_rows]
    squad_player_ids = set(
        squad.entries.filter(player_id__in=player_ids).values_list('player_id', flat=True)
    )
    if set(player_ids) - squad_player_ids:
        raise ValidationError(
            {'participants': ('Every participant must belong to the published squad.')}
        )
    players = {
        player.id: player
    ...
    if any(roster_eligibility(player, fixture.age_bracket).blocked for player in players.values()):
        raise ValidationError({'participants': 'One or more selected players are unavailable.'})
    ...
    AuditLog.record(
        ...
        detail=(f'{match.our_score}-{match.opponent_score} | {len(participant_rows)} participants'),
    )
    return match
