...
class ApprovedClubHierarchyTests(APITestCase):
    ...
    def setUp(self):
        ...
        self.school_coordinator, _ = provision_club_coordinator(
            email='school.coord@footpath.test',
            first_name='School',
        ...
        self.independent_coordinator, _ = provision_club_coordinator(
            email='independent.coord@footpath.test',
            first_name='Independent',
            last_name='Coordinator',
            club=self.independent,
            password='SafePass123!',
        )
        self.other_coordinator, _ = provision_club_coordinator(
            email='other.coord@footpath.test',
            first_name='Other',
            last_name='Coordinator',
            club=self.other,
            password='SafePass123!',
    ...
    def test_01_super_admin_can_create_club(self):
        ...
        response = self.client.post(
            ...
            {
                'name': 'Created FC',
                'club_type': ClubTypes.INDEPENDENT,
                'is_active': True,
            },
            format='json',
        )
    ...
    def test_04_super_admin_creates_coordinator_for_selected_valid_club(self):
        ...
        self.client.force_authenticate(self.super_admin)
        response = self.client.post(
            reverse('admin-coordinators'),
            {
                'email': 'selected.coord@footpath.test',
                'first_name': 'Selected',
                'last_name': 'Coordinator',
                'club_id': club.id,
                'password': 'SafePass123!',
        ...
        invalid = self.client.post(
            reverse('admin-coordinators'),
            {
                'email': 'invalid.coord@footpath.test',
                'first_name': 'Invalid',
                'last_name': 'Club',
                'club_id': 999999,
            },
    ...
    def test_07_coordinator_cannot_create_player_in_another_club(self):
        other_guardian = self._member(
            email='other.guardian07@footpath.test',
            role=Roles.GUARDIAN,
            club=self.other,
        )
        response = self._portal_post(
    ...
    def test_08_modified_club_id_cannot_bypass_server_derived_club(self):
        guardian = self._member(
            email='guardian08@footpath.test',
            role=Roles.GUARDIAN,
            club=self.school,
        )
        self._portal_post(
    ...
    def test_12_coordinator_can_link_guardian_to_same_club_player(self):
        guardian = self._member(
            email='guardian12@footpath.test',
            role=Roles.GUARDIAN,
            club=self.school,
        )
    ...
    def test_13_cross_club_guardian_link_is_rejected(self):
        guardian = self._member(
            email='guardian13@footpath.test',
            role=Roles.GUARDIAN,
            club=self.school,
        )
    ...
    def test_17_independent_club_gets_not_applicable_eligibility_behavior(self):
        player, profile = self._profile(
            email='player17@footpath.test',
            club=self.independent,
        )
        self.assertFalse(PlayerSerializer(profile).data['academicEligibilityApplicable'])
    ...
    def _assert_role_cannot_create_accounts(self, role, email):
        ...
        if role == Roles.PLAYER:
            PlayerProfile.objects.create(user=user, date_of_birth=date(2012, 1, 1))
        self.client.force_login(user)
        response = self.client.post(
            reverse('portal:create-account'),
            {
                'account_type': 'coach',
                'email': f'created-by-{email}',
                'first_name': 'Not',
                'last_name': 'Allowed',
            },
        )
